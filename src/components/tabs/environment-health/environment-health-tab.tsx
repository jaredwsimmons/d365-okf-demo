...
import { useSolutions, useSolutionComponents, useSolutionDetail, useEnvironmentComponentMatrix } from "@/hooks/use-inventory-api";
...
// --- Main Component ---
export function EnvironmentHealthTab() {
  const { navigateToTab } = useDashboard();
  const { data: apiSolutions, isLoading: solLoading } = useSolutions();
  ...
  const solIndex = useMemo<SolutionIndex | null>(() => {
    ...
    return buildSolutionIndexFromApi(
      apiSolutions as { solutions: Array<Record<string, unknown>>; dependencies: Array<Record<string, unknown>> },
      apiSolComponents as { bySolution: Record<string, { total: number; byType: Record<string, number> }> } | undefined,
    );
  }, [apiSolutions, apiSolComponents]);

  // Cross-env data
  const matrixData = apiMatrix as Record<string, unknown> | undefined;
  const envKeys: string[] = useMemo(() => {
    const meta = (matrixData?.metadata as Record<string, unknown>) || {};
    const raw: string[] = (meta.environments as string[]) || [];
    return ENV_ORDER.filter(k => raw.includes(k));
  }, [matrixData]);

  const crossEnvDiffs = useMemo<Map<string, SolutionDiff>>(() => {
    const map = new Map<string, SolutionDiff>();
    for (const d of ((matrixData?.coreSolutionDiffs as SolutionDiff[]) || [])) map.set(d.solution, d);
    for (const d of ((matrixData?.nonCoreSolutionDiffs as SolutionDiff[]) || [])) map.set(d.solution, d);
    return map;
  }, [matrixData]);
  ...
  const stats = useMemo(() => {
    ...
    return { total: solIndex.allSolutions.length, layers: activeLayers, totalComps: solIndex.totalComponents, largest, largestCount };
  }, [solIndex]);
  ...
  if (solLoading) return <ExplorerSkeleton />;
                  ...
                  return (
                    <div key={layer}>
                      ...
                      {isExpanded && solutions.map(sol => {
                        ...
                        const hasCrossEnv = crossEnvDiffs.has(sol.name);
                        return (
                          <div
                            key={sol.name}
                            className={cn(
                          ...
                          >
                            ...
                            {hasCrossEnv && (
                              <span className="text-[9px] text-amber-600 shrink-0">drift</span>
                            )}
        ...
        detailPane={
          !selection ? (
            ...
            <SolutionDetail
              ...
              solutionDeps={(apiSolutions || null) as import("@/types/inventory").SolutionDependenciesData | null}
              crossEnvDiff={crossEnvDiffs.get(selectedSolution.name) || null}
              envKeys={envKeys}
            />
          ) : null
        }
...
function LayerDetail({
  ...
  const sortedTypes = useMemo(() => {
    return Object.entries(layerInfo.countsByType).sort((a, b) => b[1] - a[1]);
  }, [layerInfo]);
...
// --- Solution Detail (combined architecture + cross-env health) ---
...
function SolutionDetail({
  ...
  crossEnvDiff: SolutionDiff | null;
  envKeys: string[];
}) {
  // Fetch the actual components for this solution on demand
  const { data: apiDetail } = useSolutionDetail(solution.name);

  // Merge API components into the solution object for rendering
  const effectiveSolution = useMemo(() => {
    if (!apiDetail?.components) return solution;
    // Map API components to IndexedComponent shape
    const comps = apiDetail.components.map((c) => ({
      name: c.name,
      type: c.type,
      itemId: c.id,
      searchName: c.id,
      ...
      sub: "",
      altId: "",
    } as import("@/lib/component-index").IndexedComponent));
    return { ...solution, components: comps };
  }, [solution, apiDetail]);

  const bpcName = effectiveSolution.bpcL1Code ? l1Lookup[effectiveSolution.bpcL1Code] : null;
  ...
  const groupedByType = useMemo(() => {
    const groups: Record<string, typeof effectiveSolution.components> = {};
    for (const c of effectiveSolution.components) {
  ...
  return (
    <ScrollArea className="flex-1 h-0">
      <div className="p-4 space-y-4">
        ...
        {crossEnvDiff && envKeys.length > 0 && (
          <>
            <div>
              <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Environment Comparison</h3>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b bg-muted/30">
                      ...
                      <th className="text-right px-3 py-2 font-medium">Components</th>
                    </tr>
                  </thead>
                  <tbody>
                    {envKeys.map(k => {
                      if (!crossEnvDiff.version[k]) return null;
                      const count = crossEnvDiff.totalComponentCounts[k] || 0;
                      const maxCount = Math.max(...Object.values(crossEnvDiff.totalComponentCounts));
                      const isMax = count === maxCount && count > 0;
                      return (
                        <tr key={k} className="border-b last:border-b-0">
                          <td className="px-3 py-1.5">{ENV_NAMES[k] || k}</td>
                          <td className="px-3 py-1.5 font-mono">{crossEnvDiff.version[k]}</td>
                          <td className={`px-3 py-1.5 text-right tabular-nums font-medium ${isMax ? "text-green-600" : "text-muted-foreground"}`}>
                            {count}
                          </td>
                        </tr>
                      );
            ...
            {/* Cross-env component type diff */}
            {crossEnvDiff.typeDiffs.length > 0 && (
              <div>
                <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Component Drift by Type</h3>
                <div className="border rounded-lg overflow-hidden">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b bg-muted/30">
                        <th className="text-left px-3 py-1.5 font-medium">Type</th>
                        {envKeys.filter(k => crossEnvDiff.version[k]).map(k => (
                          <th key={k} className="text-center px-3 py-1.5 font-medium">{ENV_NAMES[k] || k}</th>
                        ))}
                        <th className="text-center px-3 py-1.5 font-medium">Delta</th>
                      </tr>
                    </thead>
                    <tbody>
                      {/* Aggregate duplicate type rows — the PS generator can emit
                          the same type twice with complementary counts (e.g. one
                          row for solution-present envs, one for solution-absent).
                          Sum them here so the table shows a single row per type. */}
                      {Array.from(
                        crossEnvDiff.typeDiffs.reduce((acc, td) => {
                          const existing = acc.get(td.type);
                          if (existing) {
                            for (const [k, v] of Object.entries(td.counts)) {
                              existing.counts[k] = (existing.counts[k] || 0) + v;
                            }
                          } else {
                            acc.set(td.type, { type: td.type, counts: { ...td.counts } });
                          }
                          return acc;
                        }, new Map<string, { type: string; counts: Record<string, number> }>()).values()
                      ).map(td => {
                        const activeEnvs = envKeys.filter(k => crossEnvDiff.version[k]);
                        const vals = activeEnvs.map(k => td.counts[k] || 0);
                        const nonZero = vals.filter(v => v > 0);
                        const delta = nonZero.length > 0 ? Math.max(...nonZero) - Math.min(...nonZero) : 0;
                        return (
                          <tr key={td.type} className="border-b last:border-b-0">
                            <td className="px-3 py-1">{td.type}</td>
                            {activeEnvs.map(k => {
                              const v = td.counts[k] || 0;
                              const max = Math.max(...vals);
                              const min = nonZero.length > 0 ? Math.min(...nonZero) : 0;
                              const isHigh = v === max && max !== min;
                              const isLow = v === min && max !== min && v > 0;
                              return (
                                <td key={k} className={`text-center px-3 py-1 tabular-nums ${
                                  isHigh ? "text-green-600 font-medium" : isLow ? "text-red-500" : v === 0 ? "text-muted-foreground/40" : ""
                                }`}>
                                  {v}
                                </td>
                              );
                            })}
                            <td className="text-center px-3 py-1 tabular-nums text-amber-600">
                              {delta > 0 ? `+${delta}` : ""}
                            </td>
                          </tr>
                        );
